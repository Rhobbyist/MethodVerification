...
class ReportInfo(models.Model):
    ...
    Protocol_ID1 = models.CharField(max_length=32)  # 英文代码
    Protocol_ID2 = models.CharField(max_length=32)  # 中文代码
    verifytime = models.DateField(auto_now_add=True)  # 验证时间
    status = models.CharField(max_length=32)  # 报告状态
    

# 报告流程表单
class ReportFlowChart(models.Model):
    reportinfo = models.ForeignKey(ReportInfo, on_delete=models.CASCADE)  # 外键
    number = models.CharField(max_length=32)  # 操作编号
    operation = models.CharField(max_length=32)  # 操作
    operator = models.CharField(max_length=32)  # 操作人
    time = models.CharField(max_length=32)  # 操作时间
    receiver = models.CharField(max_length=32)  # 接收人
...
class Validation_Reason(models.Model):
    reportinfo = models.ForeignKey(ReportInfo, on_delete=models.CASCADE)  # 外键
    reason = models.CharField(max_length=500,blank=True)  # 验证原因
...
class Stability(models.Model):
    ...
    namestability = models.CharField(max_length=32)  # 稳定性名称：样品储存稳定性,样品处理后稳定性
...
class Repeatprecisiongeneral(models.Model):
    general = models.OneToOneField(
        General, verbose_name="方法学报告性能验证指标", on_delete=models.CASCADE, null=True)
    name = models.CharField(
        max_length=32, verbose_name="验证指标", blank=True, editable=False)
...
class Repeatprecisiongeneralmethod(models.Model):
    repeatprecisiongeneral = models.OneToOneField(
        Repeatprecisiongeneral, verbose_name="重复性精密度", on_delete=models.CASCADE)
...
class Repeatprecisiongeneraltexts(models.Model):
    repeatprecisiongeneral = models.ForeignKey(
        Repeatprecisiongeneral, verbose_name="重复性精密度", on_delete=models.CASCADE)
...
class Interprecisiongeneralmethod(models.Model):
    interprecisiongeneral = models.OneToOneField(
        Interprecisiongeneral, verbose_name="中间精密度", on_delete=models.CASCADE)
    minSample = models.IntegerField("所需最小样本数")
    maxCV = models.FloatField("最大允许CV(%)")
...
class Interprecisiongeneraltexts(models.Model):
    interprecisiongeneral = models.ForeignKey(
        Interprecisiongeneral, verbose_name="中间精密度", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500, blank=True)
...
class PTgeneralmethod(models.Model):
    pTgeneral = models.OneToOneField(
        PTgeneral, verbose_name="PT", on_delete=models.CASCADE)
...
class Recyclegeneralmethod(models.Model):
    recyclegeneral = models.OneToOneField(
        Recyclegeneral, verbose_name="加标回收率", on_delete=models.CASCADE)
...
class Recyclegeneraltexts(models.Model):
    recyclegeneral = models.ForeignKey(
        Recyclegeneral, verbose_name="加标回收率", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class AMRgeneralmethod(models.Model):
    aMRgeneral = models.OneToOneField(
        AMRgeneral, verbose_name="AMR", on_delete=models.CASCADE)
    lowvalue = models.FloatField("回收率下限(%)")
    upvalue = models.FloatField("回收率上限(%)")
...
class JCXgeneraltexts(models.Model):
    jCXgeneral = models.ForeignKey(
        JCXgeneral, verbose_name="JCX", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class CRRgeneralmethod(models.Model):
    cRRgeneral = models.OneToOneField(
        CRRgeneral, verbose_name="CRR", on_delete=models.CASCADE)
    lowvalue = models.FloatField("回收率下限(%)")
    upvalue = models.FloatField("回收率上限(%)")
...
class MSgeneraltexts(models.Model):
    mSgeneral = models.ForeignKey(
        MSgeneral, verbose_name="基质特异性", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class Carryovergeneralmethod(models.Model):
    carryovergeneral = models.OneToOneField(
        Carryovergeneral, verbose_name="携带效应", on_delete=models.CASCADE)
...
class Carryovergeneraltexts(models.Model):
    carryovergeneral = models.ForeignKey(
        Carryovergeneral, verbose_name="携带效应", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class Matrixeffectgeneralmethod(models.Model):
    matrixeffectgeneral = models.OneToOneField(
        Matrixeffectgeneral, verbose_name="基质效应", on_delete=models.CASCADE)
...
class Matrixeffectgeneraltexts(models.Model):
    matrixeffectgeneral = models.ForeignKey(
        Matrixeffectgeneral, verbose_name="基质效应", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class Stabilitygeneralmethod(models.Model):
    stabilitygeneral = models.OneToOneField(
        Stabilitygeneral, verbose_name="回收率", on_delete=models.CASCADE)
    lowvalue = models.FloatField("回收率下限(%)")
    upvalue = models.FloatField("回收率上限(%)")
...
class Stabilitygeneraltexts(models.Model):
    stabilitygeneral = models.ForeignKey(
        Stabilitygeneral, verbose_name="", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class Referenceintervalgeneraltexts(models.Model):
    referenceintervalgeneral = models.ForeignKey(
        Referenceintervalgeneral, verbose_name="", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class Special(models.Model):
    ...
    Detectionplatform = models.CharField(
        max_length=16, choices=Detectionplatform_CHOICES, verbose_name="检测平台")
    project = models.CharField(
        max_length=32, verbose_name="项目名称", unique=True)  # unique=True设置项目不可重复
    chinese_titie = models.TextField(
        max_length=200, verbose_name="中文标题", blank=True)
    english_titie = models.TextField(
        max_length=200, verbose_name="英文标题", blank=True)
...
class Repeatprecisionspecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(max_length=32, verbose_name="验证指标",
                            blank=True, default="重复性精密度", editable=False)
...
class Repeatprecisionspecialmethod(models.Model):
    repeatprecisionspecial = models.OneToOneField(
        Repeatprecisionspecial, verbose_name="重复性精密度", on_delete=models.CASCADE)
    minSample = models.IntegerField("所需最小样本数", blank=True)
...
class Repeatprecisionspecialtexts(models.Model):
    repeatprecisionspecial = models.ForeignKey(
        Repeatprecisionspecial, verbose_name="重复性精密度", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500, blank=True)
...
class PTspecialaccept(models.Model):
    ...
    loq = models.FloatField("LOQ", blank=True, null=True)
...
class PTspecialBoolean(models.Model):
    pTspecial = models.OneToOneField(
        PTspecial, verbose_name="PT", on_delete=models.CASCADE)
    Boolean = models.BooleanField(
        verbose_name="结果不通过时是否关联数据进入报告", blank=True, null=True)
...
class Recyclespecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(max_length=32, verbose_name="验证指标",
                            blank=True, default="加标回收率", editable=False)
...
class Recyclespecialmethod(models.Model):
    recyclespecial = models.OneToOneField(
        Recyclespecial, verbose_name="加标回收率", on_delete=models.CASCADE)
...
class Recyclespecialtexts(models.Model):
    recyclespecial = models.ForeignKey(
        Recyclespecial, verbose_name="加标回收率", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class AMRspecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(max_length=32, verbose_name="验证指标",
                            blank=True, default="线性灵敏度和线性范围", editable=False)
...
class AMRspecialmethod(models.Model):
    aMRspecial = models.OneToOneField(
        AMRspecial, verbose_name="AMR", on_delete=models.CASCADE)
    lowvalue = models.FloatField("回收率下限(%)", blank=True)
    upvalue = models.FloatField("回收率上限(%)", blank=True)
...
class JCXspecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(
        max_length=32, verbose_name="验证指标", blank=True, default="检出限", editable=False)
...
class JCXspecialtexts(models.Model):
    jCXspecial = models.ForeignKey(
        JCXspecial, verbose_name="JCX", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class CRRspecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(max_length=32, verbose_name="验证指标",
                            blank=True, default="临床可报告范围", editable=False)
...
class CRRspecialmethod(models.Model):
    cRRspecial = models.OneToOneField(
        CRRspecial, verbose_name="CRR", on_delete=models.CASCADE)
    lowvalue = models.FloatField("回收率下限(%)", blank=True)
    upvalue = models.FloatField("回收率上限(%)", blank=True)
...
class MSspecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(max_length=32, verbose_name="验证指标",
                            blank=True, default="基质特异性", editable=False)
...
class MSspecialtexts(models.Model):
    mSspecial = models.ForeignKey(
        MSspecial, verbose_name="基质特异性", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
class Carryoverspecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(
        max_length=32, verbose_name="验证指标", blank=True, default="携带效应", editable=False)
...
class Carryoverspecialmethod(models.Model):
    carryoverspecial = models.OneToOneField(
        Carryoverspecial, verbose_name="携带效应", on_delete=models.CASCADE)
...
class Matrixeffectspecial(models.Model):
    special = models.OneToOneField(
        Special, verbose_name="特殊参数设置", on_delete=models.CASCADE, null=True)
    name = models.CharField(
        max_length=32, verbose_name="验证指标", blank=True, default="基质效应", editable=False)
...
class Matrixeffectspecialmethod(models.Model):
    matrixeffectspecial = models.OneToOneField(
        Matrixeffectspecial, verbose_name="基质效应", on_delete=models.CASCADE)
...
class Matrixeffectspecialtexts(models.Model):
    matrixeffectspecial = models.ForeignKey(
        Matrixeffectspecial, verbose_name="基质效应", on_delete=models.CASCADE)
    text = models.TextField("描述性内容", max_length=500)
...
#     if created:
#         Repeatprecisiongeneral.objects.create(general=instance)
#         Interprecisiongeneral.objects.create(general=instance)
#         PTgeneral.objects.create(general=instance)
#         Recyclegeneral.objects.create(general=instance)
#         AMRgeneral.objects.create(general=instance)
#         JCXgeneral.objects.create(general=instance)
#         CRRgeneral.objects.create(general=instance)
#         MSgeneral.objects.create(general=instance)
#         Carryovergeneral.objects.create(general=instance)
#         Matrixeffectgeneral.objects.create(general=instance)
#         Stabilitygeneral.objects.create(general=instance)
#         Referenceintervalgeneral.objects.create(general=instance)
...
class Testmethod(models.Model):
    ...
    platform = models.CharField(
        max_length=16, choices=platform_CHOICES, verbose_name="仪器平台")
    ...
    factory = models.CharField(
        max_length=16, choices=factory_CHOICES, verbose_name="仪器厂家")
    ...
    Detectionplatform = models.CharField(
        max_length=16, choices=Detectionplatform_CHOICES, verbose_name="检测平台")
    project = models.CharField(
        max_length=16, verbose_name="项目名称", unique=True)  # 检测项目
    column = models.CharField(
        max_length=100, verbose_name="色谱柱", blank=True)  # 色谱柱
    Instrument_model = models.CharField(
        max_length=100, verbose_name="仪器型号", blank=True)  # 仪器型号
...
class YX_Method(models.Model):
    ...
    time = models.CharField(
        max_length=200, verbose_name="分析时间(min)", blank=True)
    Flowrate = models.CharField(
        max_length=200, verbose_name="流速(mL/min)", blank=True)
    Mobile_phaseA = models.CharField(
        max_length=200, verbose_name="流动相A(水相)", blank=True)
    Mobile_phaseB = models.CharField(
        max_length=200, verbose_name="流动相B(有机相)", blank=True)
...
# 适用于双系统项目，需要设置两个液相方法表格


class YX_Method2(models.Model):
    testmethod = models.ForeignKey(Testmethod, on_delete=models.CASCADE)
    step = models.CharField(max_length=200, verbose_name="步骤", blank=True)
    time = models.CharField(
        max_length=200, verbose_name="分析时间(min)", blank=True)
    ...
    class Meta:
        verbose_name = "液相方法2(双系统仪器填写)"
        verbose_name_plural = "液相方法2(双系统仪器填写)"
...
class Detection_equipment(models.Model):
    equipment = models.ForeignKey(
        Equipment, verbose_name="检测设备", on_delete=models.CASCADE, null=True)
    text = models.TextField("描述性内容", max_length=500)
...
class Auxiliary_equipment(models.Model):
    equipment = models.ForeignKey(
        Equipment, verbose_name="辅助设备", on_delete=models.CASCADE, null=True)
    text = models.TextField("描述性内容", max_length=500)
...
class ReagentsTable(models.Model):
    reagents_Consumables = models.ForeignKey(
        Reagents_Consumables, verbose_name="主要试剂", on_delete=models.CASCADE, null=True)
    Col1 = models.CharField(max_length=200, verbose_name="第1列", blank=True)
    Col2 = models.CharField(max_length=200, verbose_name="第2列", blank=True)
    Col3 = models.CharField(max_length=200, verbose_name="第3列", blank=True)
    Col4 = models.CharField(max_length=200, verbose_name="第4列", blank=True)
    Col5 = models.CharField(max_length=200, verbose_name="第5列", blank=True)
    Col6 = models.CharField(max_length=200, verbose_name="第6列", blank=True)
    Col7 = models.CharField(max_length=200, verbose_name="第7列", blank=True)
...
class Consumables(models.Model):
    reagents_Consumables = models.ForeignKey(
        Reagents_Consumables, verbose_name="主要耗材", on_delete=models.CASCADE, null=True)
    text = models.TextField("描述性内容", max_length=500)
...
class Profile(models.Model):
    ...
    Detectionplatform_CHOICES = (
        ...
        ('治疗药物检测平台', '治疗药物检测平台'),
        ('研发与创新平台', '研发与创新平台'),
        ('内分泌检测平台&研发与创新平台', '内分泌检测平台&研发与创新平台'),
        ('临床服务一部', '临床服务一部'),
        ('临床服务二部', '临床服务二部'),
        ('中心负责人 ', '中心负责人 '),
    )
    Detectionplatform = models.CharField(
        max_length=16, choices=Detectionplatform_CHOICES, verbose_name="检测平台", blank=True)
    ...
    Group = models.CharField(max_length=16, choices=Group_CHOICES, verbose_name="项目组", blank=True)
    ...
    class Meta:
        ...
        verbose_name_plural = "所属检测平台及项目组"
...
class BulkGetOrCreateManager(models.Manager):
    def bulk_get_or_create(self, objs, lookup_field=None):
        ...
        lookup = {f'{lookup_field}__in': [
            getattr(obj, lookup_field) for obj in objs]}
        existing_objects = [
