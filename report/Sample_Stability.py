...
# 样品储存稳定性
def store_fileread(files, namestability,Detectionplatform,reportinfo, project, platform, manufacturers, Unit, digits, ZP_Method_precursor_ion, ZP_Method_product_ion, normAB, Number_of_compounds):
      ...
      for file in files:
            if platform=="液质":
                  if manufacturers =="Agilent":
                        ...
                        # 定义各验证条件下(室温，冷藏，冷冻)的时间列表，0h为默认添加的
                        Room_tem_list = ["0"]
                        Refrigerate_tem_list = ["0"]
                        Freeze_tem_list = ["0"]
                        ...
                        for i in range(len(lines)): # 循环原始数据中的每一行                   
                              if "Poststability" in lines[i][nameindex]: # 处理后稳定性，命名前必须含有关键字段“Poststability”

                                    # 1 含有关键字段"RT",说明为室温(samplename格式固定为“Poststability-L-5h-RT-1”,其中“5”为时间)
                                    if "RT" in lines[i][nameindex]:  

                                          # 1.1 添加浓度水平
                                          # 不论samplename长度为多少，浓度水平固定为第14位
                                          if Conclevel_dict[lines[i][nameindex][14:15]] not in Room_conclevel_list: # 避免重复加入列表
                                                Room_conclevel_list.append(Conclevel_dict[lines[i][nameindex][14:15]])  # 取出浓度水平位放入列表中
                                          
                                          # 1.2 添加时间
                                          if len(lines[i][nameindex].split("h")[0]) == 17: # 依据上述格式，以“h”为切割点进行切割后，若长度为17，说明时间仅有一位
                                                if lines[i][nameindex][16:17] not in Room_tem_list: # 避免重复加入列表
                                                      Room_tem_list.append(lines[i][nameindex][16:17])  # 取出时间位放入列表中                                   
                                          elif len(lines[i][nameindex].split("h")[0]) == 18: # # 依据上述格式，以“h”为切割点进行切割后，若长度为18，说明时间有两位
                                                if lines[i][nameindex][16:18] not in Room_tem_list: # 避免重复加入列表
                                                      Room_tem_list.append(lines[i][nameindex][16:18])  # 取出时间位放入列表中

                                    # 2 含有关键字段"RE",说明为冷藏(samplename格式固定为“Poststability-L-5h-RE-1”,其中“5”为时间)
                                    elif "RE" in lines[i][nameindex]:  

                                          # 2.1 添加浓度水平
                                          # 不论samplename长度为多少，浓度水平固定为第14位
                                          if Conclevel_dict[lines[i][nameindex][14:15]] not in Refrigerate_conclevel_list: # 避免重复加入列表
                                                Refrigerate_conclevel_list.append(Conclevel_dict[lines[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                          # 2.2 添加时间
                                          if len(lines[i][nameindex].split("h")[0]) == 17: 
                                                if lines[i][nameindex][16:17] not in Refrigerate_tem_list: # 避免重复加入列表
                                                      Refrigerate_tem_list.append(lines[i][nameindex][16:17])  # 取出时间位放入列表中
                                          elif len(lines[i][nameindex].split("h")[0]) == 18: 
                                                if lines[i][nameindex][16:18] not in Refrigerate_tem_list: # 避免重复加入列表
                                                      Refrigerate_tem_list.append(lines[i][nameindex][16:18])  # 取出时间位放入列表中

                                    # 3 含有关键字段"FR",说明为冷冻(samplename格式固定为“Poststability-L-5h-FR-1”,其中“5”为时间)
                                    elif "FR" in lines[i][nameindex]:  

                                          # 2.1 添加浓度水平
                                          # 不论samplename长度为多少，浓度水平固定为第14位
                                          if Conclevel_dict[lines[i][nameindex][14:15]] not in Freeze_conclevel_list: # 避免重复加入列表
                                                Freeze_conclevel_list.append(Conclevel_dict[lines[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                          # 2.2 添加时间
                                          if len(lines[i][nameindex].split("h")[0]) == 17: # 依据上述格式，长度为17，说明时间仅有一位
                                                if lines[i][nameindex][16:17] not in Freeze_tem_list: # 避免重复加入列表
                                                      Freeze_tem_list.append(lines[i][nameindex][16:17])  # 取出时间位放入列表中
                                          elif len(lines[i][nameindex].split("h")[0]) == 18: # 依据上述格式，长度为18，说明时间有两位
                                                if lines[i][nameindex][16:18] not in Freeze_tem_list: # 避免重复加入列表
                                                      Freeze_tem_list.append(lines[i][nameindex][16:18])  # 取出时间位放入列表中
                        ...
                        for m in range(len(norm)):  # 循环每个化合物
                              ...
                              for n in Room_tem_list: # 先循环室温条件下的时间列表
                                    ...
                                    if n=="0":
                                          for q in range(len(lines)):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in lines[q][nameindex] and "0h" in lines[q][nameindex]:                                                                                    
                                                      time_list.append(effectnum(lines[q][concindex[m]],digits))  # 取出浓度放入每个时间列表中,需转换为float格式
                                    else:
                                          for q in range(len(lines)):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in lines[q][nameindex] and "RT" in lines[q][nameindex]: # 判断是否为室温条件                                                                     
                                                      if len(lines[q][nameindex].split("h")[0]) == 17: 
                                                            if lines[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间                  
                                                                  time_list.append(effectnum(lines[q][concindex[m]],digits))
                                                      elif len(lines[q][nameindex].split("h")[0]) == 18: 
                                                            if lines[q][nameindex][16:18] == n: 
                                                                  time_list.append(effectnum(lines[q][concindex[m]],digits))
                              ...
                              for n in Refrigerate_tem_list: 
                                    ...
                                    if n=="0":
                                          ...
                                          for q in range(len(lines)):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in lines[q][nameindex] and "RE" in lines[q][nameindex]: # 判断是否为冷藏条件                                                                     
                                                      if len(lines[q][nameindex].split("h")[0]) == 17: 
                                                            if lines[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                  time_list.append(effectnum(lines[q][concindex[m]],digits))
                                                      elif len(lines[q][nameindex].split("h")[0]) == 18: 
                              ...
                              for n in Freeze_tem_list: 
                                    ...
                                    if n=="0":
                                          ...
                                          for q in range(len(lines)):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in lines[q][nameindex] and "FR" in lines[q][nameindex]:                                                                   
                                                      if len(lines[q][nameindex].split("h")[0]) == 17: 
                                                            if lines[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                  time_list.append(effectnum(lines[q][concindex[m]],digits))
                                                      elif len(lines[q][nameindex].split("h")[0]) == 18:
                        ...
                        for m in range(len(norm)):  # 循环每个化合物
                              ...
                              if m < len(norm)-1:  # 如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                                    for i in range(norm_row[m], norm_row[m+1]):
                                          if "Stability" in content[i][nameindex]: 
                                                # 判断时间为所处的索引
                                                long = len(content[i][nameindex].split("h")[0])
                                                short = len(content[i][nameindex].split("y")[0])

                                                # 1 含有关键字段"RT",说明为室温(samplename格式固定为“Stability-L-5h-RT-1”,其中“5”为时间)
                                                if "RT" in content[i][nameindex]:  

                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第10位
                                                      if Conclevel_dict[content[i][nameindex][10:11]] not in Room_conclevel_list: # 避免重复加入列表
                                                            Room_conclevel_list.append(Conclevel_dict[content[i][nameindex][10:11]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if content[i][nameindex][short+4:long] not in Room_tem_list: # 避免重复加入列表
                                                            Room_tem_list.append(content[i][nameindex][short+4:long])  # 取出时间位放入列表中

                                                # 含有关键字段"RE",说明为冷藏
                                                elif "RE" in content[i][nameindex]: 
                                                      
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[content[i][nameindex][10:11]] not in Refrigerate_conclevel_list: # 避免重复加入列表
                                                            Refrigerate_conclevel_list.append(Conclevel_dict[content[i][nameindex][10:11]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if content[i][nameindex][short+4:long] not in Refrigerate_tem_list: # 避免重复加入列表
                                                            Refrigerate_tem_list.append(content[i][nameindex][short+4:long])  # 取出时间位放入列表中

                                                # 3 含有关键字段"FR",说明为冷冻
                                                elif "FR" in content[i][nameindex]: 
                                                      
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[content[i][nameindex][10:11]] not in Freeze_conclevel_list: # 避免重复加入列表
                                                            Freeze_conclevel_list.append(Conclevel_dict[content[i][nameindex][10:11]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if content[i][nameindex][short+4:long] not in Freeze_tem_list: # 避免重复加入列表
                                                            Freeze_tem_list.append(content[i][nameindex][short+4:long])  # 取出时间位放入列表中
                                    ...
                                    for n in Room_tem_list: # 先循环室温条件下的时间列表
                                          ...
                                          if n=="0":
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      if "Stability" in content[q][nameindex] and "0h" in content[q][nameindex]:                                                                                    
                                                            time_list.append(effectnum(content[q][concindex],digits))  # 取出浓度放入每个时间列表中,需转换为float格式

                                          else:
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                                                      # 判断时间为所处的索引
                                                      long = len(content[q][nameindex].split("h")[0])
                                                      short = len(content[q][nameindex].split("y")[0])

                                                      if "Stability" in content[q][nameindex] and "RT" in content[q][nameindex]: # 判断是否为室温条件                                                                     
                                                            if content[q][nameindex][short+4:long] == n: # 判断时间位是否等于循环中的时间 
                                    ...
                                    for n in Refrigerate_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      ...
                                                      if "Stability" in content[q][nameindex] and "RE" in content[q][nameindex]: # 判断是否为室温条件                                                                     
                                                            if content[q][nameindex][short+4:long] == n: # 判断时间位是否等于循环中的时间 
                                                                  time_list.append(effectnum(content[q][concindex],digits))
                                    ...
                                    for n in Freeze_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      ...
                                                      if "Stability" in content[q][nameindex] and "FR" in content[q][nameindex]: # 判断是否为室温条件                                                                     
                                                            if content[q][nameindex][short+4:long] == n: # 判断时间位是否等于循环中的时间 
                                                                  time_list.append(effectnum(content[q][concindex],digits))
                                    ...
                                    for n in Refrigerate_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], len(content)):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      ...
                                                      if "Stability" in content[q][nameindex] and "RE" in content[q][nameindex]: # 判断是否为冷藏条件                                                                     
                                                            if content[q][nameindex][short+4:long] == n: # 判断时间位是否等于循环中的时间 
                                                                  time_list.append(effectnum(content[q][concindex],digits))
                                    ...
                                    for n in Freeze_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], len(content)):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      ...
                                                      if "Stability" in content[q][nameindex] and "FR" in content[q][nameindex]: # 判断是否为冷藏条件                                                                     
                                                            if content[q][nameindex][short+4:long] == n: # 判断时间位是否等于循环中的时间 
                                                                  time_list.append(effectnum(content[q][concindex],digits))
                        ...
                        for m in range(len(norm)):  # 循环每个化合物
                              ...
                              if m < len(norm)-1:  # 如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                                    for i in range(norm_row[m], norm_row[m+1]):
                                          if "Poststability" in file_data.row_values(i)[nameindex]: 
                                                ...
                                                if "RT" in file_data.row_values(i)[nameindex]:  
                                                      ...
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[file_data.row_values(i)[nameindex][14:15]] not in Room_conclevel_list: # 避免重复加入列表
                                                            Room_conclevel_list.append(Conclevel_dict[file_data.row_values(i)[nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if len(file_data.row_values(i)[nameindex].split("h")[0]) == 17: 
                                                            if file_data.row_values(i)[nameindex][16:17] not in Room_tem_list: # 避免重复加入列表
                                                                  Room_tem_list.append(file_data.row_values(i)[nameindex][16:17])  # 取出时间位放入列表中
                                                      elif len(file_data.row_values(i)[nameindex].split("h")[0]) == 18: 
                                                            if file_data.row_values(i)[nameindex][16:18] not in Room_tem_list: # 避免重复加入列表
                                                                  Room_tem_list.append(file_data.row_values(i)[nameindex][16:18])  # 取出时间位放入列表中

                                                # 含有关键字段"RE",说明为冷藏
                                                elif "RE" in file_data.row_values(i)[nameindex]: 
                                                      
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[file_data.row_values(i)[nameindex][14:15]] not in Refrigerate_conclevel_list: # 避免重复加入列表
                                                            Refrigerate_conclevel_list.append(Conclevel_dict[file_data.row_values(i)[nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if len(file_data.row_values(i)[nameindex].split("h")[0]) == 17:
                                                            if file_data.row_values(i)[nameindex][16:17] not in Refrigerate_tem_list: # 避免重复加入列表
                                                                  Refrigerate_tem_list.append(file_data.row_values(i)[nameindex][16:17])  # 取出时间位放入列表中
                                                      elif len(file_data.row_values(i)[nameindex].split("h")[0]) == 18:
                                                            if file_data.row_values(i)[nameindex][16:18] not in Refrigerate_tem_list: # 避免重复加入列表
                                                                  Refrigerate_tem_list.append(file_data.row_values(i)[nameindex][16:18])  # 取出时间位放入列表中

                                                # 3 含有关键字段"FR",说明为冷冻
                                                elif "FR" in file_data.row_values(i)[nameindex]: 
                                                      
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[file_data.row_values(i)[nameindex][14:15]] not in Freeze_conclevel_list: # 避免重复加入列表
                                                            Freeze_conclevel_list.append(Conclevel_dict[file_data.row_values(i)[nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if len(file_data.row_values(i)[nameindex].split("h")[0]) == 17:
                                                            if file_data.row_values(i)[nameindex][16:17] not in Freeze_tem_list: # 避免重复加入列表
                                                                  Freeze_tem_list.append(file_data.row_values(i)[nameindex][16:17])  # 取出时间位放入列表中
                                                      elif len(file_data.row_values(i)[nameindex].split("h")[0]) == 18:
                                                            if file_data.row_values(i)[nameindex][16:18] not in Freeze_tem_list: # 避免重复加入列表
                                                                  Freeze_tem_list.append(file_data.row_values(i)[nameindex][16:18])  # 取出时间位放入列表中
                                    ...
                                    for n in Room_tem_list: # 先循环室温条件下的时间列表
                                          ...
                                          if n=="0":
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      if "Poststability" in file_data.row_values(q)[nameindex] and "0h" in file_data.row_values(q)[nameindex]:                                                                                    
                                                            time_list.append(effectnum(file_data.row_values(q)[concindex],digits))  # 取出浓度放入每个时间列表中,需转换为float格式

                                          else:
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                                                      if "Poststability" in file_data.row_values(q)[nameindex] and "RT" in file_data.row_values(q)[nameindex]: # 判断是否为室温条件                                                                     
                                                            if len(file_data.row_values(q)[nameindex].split("h")[0]) == 17:
                                                                  if file_data.row_values(q)[nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间                  
                                                                        time_list.append(effectnum(file_data.row_values(q)[concindex],digits))
                                                            elif len(file_data.row_values(q)[nameindex].split("h")[0]) == 18:
                                                                  if file_data.row_values(q)[nameindex][16:18] == n: 
                                                                        time_list.append(effectnum(file_data.row_values(q)[concindex],digits))
                                    ...
                                    for n in Refrigerate_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      if "Poststability" in file_data.row_values(q)[nameindex] and "RE" in file_data.row_values(q)[nameindex]: # 判断是否为冷藏条件                                                                     
                                                            if len(file_data.row_values(q)[nameindex].split("h")[0]) == 17:
                                                                  if file_data.row_values(q)[nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                        time_list.append(effectnum(file_data.row_values(q)[concindex],digits))
                                                            elif len(file_data.row_values(q)[nameindex].split("h")[0]) == 18:
                                    ...
                                    for n in Freeze_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      if "Poststability" in file_data.row_values(q)[nameindex] and "FR" in file_data.row_values(q)[nameindex]: # 判断是否为冷藏条件                                                                     
                                                            if len(file_data.row_values(q)[nameindex].split("h")[0]) == 17:
                                                                  if file_data.row_values(q)[nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                        time_list.append(effectnum(file_data.row_values(q)[concindex],digits))
                                                            elif len(file_data.row_values(q)[nameindex].split("h")[0]) == 18:
                  ...
                  # Thermo厂家需先在后台管理系统中设置本项目的化合物名称，以便查找上传文件中相应化合物的表格
                        ...
                        for index in range(len(sheetindex)):
                              ...
                              # 确定每个验证条件下（室温，冷藏，冷冻）的浓度水平和验证时间 
                              for i in range(nrows):
                                    if "Poststability" in file_data.row_values(i)[nameindex]: # 处理后稳定性，命名前必须含有关键字段“Poststability”
                                          ...
                                          if "RT" in file_data.row_values(i)[nameindex]:  
                                                ...
                                                # 1.2 添加时间
                                                if len(file_data.row_values(i)[nameindex].split("h")[0]) == 17: # 依据上述格式，以“h”为切割点进行切割后，若长度为17，说明时间仅有一位
                                                      if file_data.row_values(i)[nameindex][16:17] not in Room_tem_list: # 避免重复加入列表
                                                            Room_tem_list.append(file_data.row_values(i)[nameindex][16:17])  # 取出时间位放入列表中
                                                elif len(file_data.row_values(i)[nameindex].split("h")[0]) == 18: # 依据上述格式，以“h”为切割点进行切割后，若长度为18，说明时间有两位
                                                      if file_data.row_values(i)[nameindex][16:18] not in Room_tem_list: # 避免重复加入列表
                                                            Room_tem_list.append(file_data.row_values(i)[nameindex][16:18])  # 取出时间位放入列表中
                              ...
                              for n in Freeze_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                    ...
                                    norm_Freeze_tem_dict[n] = time_list
                              Freeze_tem_dict[norm[index]] = norm_Freeze_tem_dict
                  ...
                  # 后台管理系统中进行设置时，化合物的设置顺序需与文件中的化合物排列顺序一致，即从上向下进行设置
                        ...
                        for k in range(len(tableindex)):
                              ...
                              for i in range(len(rowdatagatherlist)):
                                    rowdatagatherlist[i][nameindex]

                                    if "Poststability" in rowdatagatherlist[i][nameindex]: # 处理后稳定性，命名前必须含有关键字段“Poststability”
                                          ...
                                          if "RT" in rowdatagatherlist[i][nameindex]:  
                                                ...
                                                # 1.1 添加浓度水平
                                                # 不论samplename长度为多少，浓度水平固定为第14位
                                                if Conclevel_dict[rowdatagatherlist[i][nameindex][14:15]] not in Room_conclevel_list: # 避免重复加入列表
                                                      Room_conclevel_list.append(Conclevel_dict[rowdatagatherlist[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                # 1.2 添加时间
                                                if len(rowdatagatherlist[i][nameindex].split("h")[0]) == 17: # 依据上述格式，以“h”为切割点进行切割后，若长度为17，说明时间仅有一位
                                                      if rowdatagatherlist[i][nameindex][16:17] not in Room_tem_list: # 避免重复加入列表
                                                            Room_tem_list.append(rowdatagatherlist[i][nameindex][16:17])  # 取出时间位放入列表中
                                                elif len(rowdatagatherlist[i][nameindex].split("h")[0]) == 18: # 依据上述格式，以“h”为切割点进行切割后，若长度为18，说明时间有两位
                                                      if rowdatagatherlist[i][nameindex][16:18] not in Room_tem_list: # 避免重复加入列表
                                                            Room_tem_list.append(rowdatagatherlist[i][nameindex][16:18])  # 取出时间位放入列表中

                                          # 2 含有关键字段"RE",说明为冷藏(samplename格式固定为“Poststability-L-5h-RE-1”,其中“5”为时间)
                                          elif "RE" in rowdatagatherlist[i][nameindex]:  

                                                # 1.1 添加浓度水平
                                                # 不论samplename长度为多少，浓度水平固定为第14位
                                                if Conclevel_dict[rowdatagatherlist[i][nameindex][14:15]] not in Refrigerate_conclevel_list: # 避免重复加入列表
                                                      Refrigerate_conclevel_list.append(Conclevel_dict[rowdatagatherlist[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                # 1.2 添加时间
                                                if len(rowdatagatherlist[i][nameindex].split("h")[0]) == 17: # 依据上述格式，以“h”为切割点进行切割后，若长度为17，说明时间仅有一位
                                                      if rowdatagatherlist[i][nameindex][16:17] not in Refrigerate_tem_list: # 避免重复加入列表
                                                            Refrigerate_tem_list.append(rowdatagatherlist[i][nameindex][16:17])  # 取出时间位放入列表中
                                                elif len(rowdatagatherlist[i][nameindex].split("h")[0]) == 18: # 依据上述格式，以“h”为切割点进行切割后，若长度为18，说明时间有两位
                                                      if rowdatagatherlist[i][nameindex][16:18] not in Refrigerate_tem_list: # 避免重复加入列表
                                                            Refrigerate_tem_list.append(rowdatagatherlist[i][nameindex][16:18])  # 取出时间位放入列表中
                                          
                                          # 3 含有关键字段"FR",说明为冷冻(samplename格式固定为“Poststability-L-5h-FR-1”,其中“5”为时间)
                                          elif "FR" in rowdatagatherlist[i][nameindex]:  

                                                # 1.1 添加浓度水平
                                                # 不论samplename长度为多少，浓度水平固定为第14位
                                                if Conclevel_dict[rowdatagatherlist[i][nameindex][14:15]] not in Freeze_conclevel_list: # 避免重复加入列表
                                                      Freeze_conclevel_list.append(Conclevel_dict[rowdatagatherlist[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                # 1.2 添加时间
                                                if len(rowdatagatherlist[i][nameindex].split("h")[0]) == 17: # 依据上述格式，以“h”为切割点进行切割后，若长度为17，说明时间仅有一位
                                                      if rowdatagatherlist[i][nameindex][16:17] not in Freeze_tem_list: # 避免重复加入列表
                                                            Freeze_tem_list.append(rowdatagatherlist[i][nameindex][16:17])  # 取出时间位放入列表中
                                                elif len(rowdatagatherlist[i][nameindex].split("h")[0]) == 18: # 依据上述格式，以“h”为切割点进行切割后，若长度为18，说明时间有两位
                                                      if rowdatagatherlist[i][nameindex][16:18] not in Freeze_tem_list: # 避免重复加入列表
                                                            Freeze_tem_list.append(rowdatagatherlist[i][nameindex][16:18])  # 取出时间位放入列表中
                              ...
                              for n in Room_tem_list: # 先循环室温条件下的时间列表
                                    ...
                                    if n=="0":
                                          for q in range(nrows):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in rowdatagatherlist[q][nameindex] and "0h" in rowdatagatherlist[q][nameindex]:                                                                                    
                                                      time_list.append(effectnum(rowdatagatherlist[q][concindex],digits))  # 取出浓度放入每个时间列表中,需转换为float格式
                                    
                                    else:
                                          for q in range(nrows):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in rowdatagatherlist[q][nameindex] and "RT" in rowdatagatherlist[q][nameindex]: # 判断是否为室温条件                                                                     
                                                      if len(rowdatagatherlist[i][nameindex].split("h")[0]) == 17:
                                                            if rowdatagatherlist[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间                  
                                                                  time_list.append(effectnum(rowdatagatherlist[q][concindex],digits))  # 取出浓度放入每个时间列表中,需转换为float格式
                                                      elif len(rowdatagatherlist[i][nameindex].split("h")[0]) == 18:
                                                            if rowdatagatherlist[q][nameindex][16:18] == n: 
                                                                  time_list.append(effectnum(rowdatagatherlist[q][concindex],digits))
                              ...
                              for n in Refrigerate_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                    ...
                                    if n=="0":
                                          ...
                                          for q in range(nrows):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in rowdatagatherlist[q][nameindex] and "RE" in rowdatagatherlist[q][nameindex]: # 判断是否为冷藏条件                                                                     
                                                      if len(rowdatagatherlist[i][nameindex].split("h")[0]) == 17:
                                                            if rowdatagatherlist[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                  time_list.append(effectnum(rowdatagatherlist[q][concindex],digits))
                                                      elif len(rowdatagatherlist[i][nameindex].split("h")[0]) == 18:
                              ...
                              for n in Freeze_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                    ...
                                    if n=="0":
                                          ...
                                          for q in range(nrows):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                if "Poststability" in rowdatagatherlist[q][nameindex] and "FR" in rowdatagatherlist[q][nameindex]: # 判断是否为冷藏条件                                                                     
                                                      if len(rowdatagatherlist[i][nameindex].split("h")[0]) == 17:
                                                            if rowdatagatherlist[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                  time_list.append(effectnum(rowdatagatherlist[q][concindex],digits))
                                                      elif len(rowdatagatherlist[i][nameindex].split("h")[0]) == 18:
      ...
      print("Room_tem_dict:%s" % (Room_tem_dict))
      print("Refrigerate_tem_dict:%s" % (Refrigerate_tem_dict))
      print("Freeze_tem_dict:%s" % (Freeze_tem_dict))
      ...
      return {"namestability": namestability,"Room_conclevel_list": Room_conclevel_list,"Room_tem_dict": Room_tem_dict, 
...
# 样品处理后稳定性
def handle_fileread(files, namestability,Detectionplatform,reportinfo, project, platform, manufacturers, Unit, digits, ZP_Method_precursor_ion, ZP_Method_product_ion, normAB, Number_of_compounds):
            ...
            if platform=="液质":
                  if manufacturers =="Agilent":
                        ...
                        for m in range(len(norm)):  # 循环每个化合物
                              ...
                              if m < len(norm)-1:  # 如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                                    for i in range(norm_row[m], norm_row[m+1]):
                                          if "Poststability" in content[i][nameindex]: 
                                                ...
                                                if "RT" in content[i][nameindex]:  
                                                      ...
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[content[i][nameindex][14:15]] not in Room_conclevel_list: # 避免重复加入列表
                                                            Room_conclevel_list.append(Conclevel_dict[content[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if len(content[i][nameindex].split("h")[0]) == 17: 
                                                            if content[i][nameindex][16:17] not in Room_tem_list: # 避免重复加入列表
                                                                  Room_tem_list.append(content[i][nameindex][16:17])  # 取出时间位放入列表中
                                                      elif len(content[i][nameindex].split("h")[0]) == 18: 
                                                            if content[i][nameindex][16:18] not in Room_tem_list: # 避免重复加入列表
                                                                  Room_tem_list.append(content[i][nameindex][16:18])  # 取出时间位放入列表中
                                                ...
                                                # 含有关键字段"RE",说明为冷藏
                                                      ...
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[content[i][nameindex][14:15]] not in Refrigerate_conclevel_list: # 避免重复加入列表
                                                            Refrigerate_conclevel_list.append(Conclevel_dict[content[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if len(content[i][nameindex].split("h")[0]) == 17:
                                                            if content[i][nameindex][16:17] not in Refrigerate_tem_list: # 避免重复加入列表
                                                                  Refrigerate_tem_list.append(content[i][nameindex][16:17])  # 取出时间位放入列表中
                                                      elif len(content[i][nameindex].split("h")[0]) == 18:
                                                            if content[i][nameindex][16:18] not in Refrigerate_tem_list: # 避免重复加入列表
                                                                  Refrigerate_tem_list.append(content[i][nameindex][16:18])  # 取出时间位放入列表中
                                                ...
                                                # 3 含有关键字段"FR",说明为冷冻
                                                      ...
                                                      # 1.1 添加浓度水平
                                                      # 不论samplename长度为多少，浓度水平固定为第14位
                                                      if Conclevel_dict[content[i][nameindex][14:15]] not in Freeze_conclevel_list: # 避免重复加入列表
                                                            Freeze_conclevel_list.append(Conclevel_dict[content[i][nameindex][14:15]])  # 取出浓度水平位放入列表中

                                                      # 1.2 添加时间
                                                      if len(content[i][nameindex].split("h")[0]) == 17:
                                                            if content[i][nameindex][16:17] not in Freeze_tem_list: # 避免重复加入列表
                                                                  Freeze_tem_list.append(content[i][nameindex][16:17])  # 取出时间位放入列表中
                                                      elif len(content[i][nameindex].split("h")[0]) == 18:
                                                            if content[i][nameindex][16:18] not in Freeze_tem_list: # 避免重复加入列表
                                                                  Freeze_tem_list.append(content[i][nameindex][16:18])  # 取出时间位放入列表中
                                    ...
                                    for n in Room_tem_list: # 先循环室温条件下的时间列表
                                          ...
                                          if n=="0":
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      if "Poststability" in content[q][nameindex] and "0h" in content[q][nameindex]:                                                                                    
                                                            time_list.append(effectnum(content[q][concindex],digits))  # 取出浓度放入每个时间列表中,需转换为float格式

                                          else:
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                                                      if "Poststability" in content[q][nameindex] and "RT" in content[q][nameindex]: # 判断是否为室温条件                                                                     
                                                            if len(content[q][nameindex].split("h")[0]) == 17:
                                                                  if content[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间                  
                                                                        time_list.append(effectnum(content[q][concindex],digits))
                                                            elif len(content[q][nameindex].split("h")[0]) == 18:
                                                                  if content[q][nameindex][16:18] == n: 
                                                                        time_list.append(effectnum(content[q][concindex],digits))
                                    ...
                                    for n in Refrigerate_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      if "Poststability" in content[q][nameindex] and "RE" in content[q][nameindex]: # 判断是否为冷藏条件                                                                     
                                                            if len(content[q][nameindex].split("h")[0]) == 17:
                                                                  if content[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                        time_list.append(effectnum(content[q][concindex],digits))
                                                            elif len(content[q][nameindex].split("h")[0]) == 18:
                                    ...
                                    for n in Freeze_tem_list: # 再循环冷藏条件下的时间列表，先写两个，后面需要加上冷冻条件
                                          ...
                                          if n=="0":
                                                ...
                                                for q in range(norm_row[m], norm_row[m+1]):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                      if "Poststability" in content[q][nameindex] and "FR" in content[q][nameindex]: # 判断是否为冷藏条件                                                                     
                                                            if len(content[q][nameindex].split("h")[0]) == 17:
                                                                  if content[q][nameindex][16:17] == n:  # 判断时间位是否等于循环中的时间
                                                                        time_list.append(effectnum(content[q][concindex],digits))
                                                            elif len(content[q][nameindex].split("h")[0]) == 18:
                  ...
                  # Thermo厂家需先在后台管理系统中设置本项目的化合物名称，以便查找上传文件中相应化合物的表格
                        ...
                        for index in range(len(sheetindex)):
                              ...
                              for i in range(nrows):
                                    if "Poststability" in file_data.row_values(i)[nameindex]: # 处理后稳定性，命名前必须含有关键字段“Poststability”
                                          # 判断时间为所处的索引
                                          long = len(file_data.row_values(i)[nameindex].split("h")[0])
                                          short = len(file_data.row_values(i)[nameindex].split("y")[0])
                                          ...
                                          if "RT" in file_data.row_values(i)[nameindex]:  
                                                ...
                                                # 1.2 添加时间                 
                                                if file_data.row_values(i)[nameindex][short+4:long] not in Room_tem_list: # 避免重复加入列表
                                                      Room_tem_list.append(file_data.row_values(i)[nameindex][short+4:long])  # 取出时间位放入列表中
                                          ...
                                          # 2 含有关键字段"RE",说明为冷藏(samplename格式固定为“Poststability-L-5h-RE-1”,其中“5”为时间)
                                                ...
                                                # 1.2 添加时间                 
                                                if file_data.row_values(i)[nameindex][short+4:long] not in Refrigerate_tem_list: # 避免重复加入列表
                                                      Refrigerate_tem_list.append(file_data.row_values(i)[nameindex][short+4:long])  # 取出时间位放入列表中
                                          ...
                                          # 3 含有关键字段"FR",说明为冷冻(samplename格式固定为“Poststability-L-5h-FR-1”,其中“5”为时间)
                                                ...
                                                # 1.2 添加时间                 
                                                if file_data.row_values(i)[nameindex][short+4:long] not in Freeze_tem_list: # 避免重复加入列表
                                                      Freeze_tem_list.append(file_data.row_values(i)[nameindex][short+4:long])  # 取出时间位放入列表中
                              ...
                              for n in Room_tem_list: # 先循环室温条件下的时间列表
                                    ...
                                    if n=="0":
                                          ...
                                          for q in range(nrows):  # 循环每一行，后续优化时可以判断每个验证指标从哪一行开始就从哪一行开始循环，减少循环数据量
                                                # 判断时间为所处的索引
                                                long = len(file_data.row_values(q)[nameindex].split("h")[0])
                                                short = len(file_data.row_values(q)[nameindex].split("y")[0])

                                                if "Poststability" in file_data.row_values(q)[nameindex] and "RT" in file_data.row_values(q)[nameindex]: # 判断是否为室温条件 
                                                      if file_data.row_values(q)[nameindex][short+4:long] == n: # 判断时间位是否等于循环中的时间 
                                                            time_list.append(effectnum(file_data.row_values(q)[concindex],digits))
                  ...
                  for index,k in enumerate(middle_table):
                        ...
                        if index == 0:
                              ...
                              if "/" not in [k.L01,k.L02,k.L03,k.M01,k.M02,k.M03,k.H01,k.H02,k.H03]:
                                    ...
                                    # middle_list2 = [effectnum(c,digits) for c in middle_list]
                                    middle_dict[str(int(k.time))]=middle_list
                                    middle_list=[] # 重置列表
