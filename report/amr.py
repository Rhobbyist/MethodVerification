...
# LOQ数据抓取
# 1 上传单个数据文件
def LOQfileread(files,reportinfo,project,platform,Detectionplatform,manufacturers,Unit,digits,ZP_Method_precursor_ion,ZP_Method_product_ion,normAB,Number_of_compounds):
            ...
            if platform=="液质":
                if manufacturers =="Agilent":                             
                    ...
                    if Detectionplatform=="治疗药物检测平台":
                        ...
                        # 删除theoryconindex、concindex和accuracyindex中的dxindex
                        theoryconindex=[theoryconindex[i] for i in range(len(theoryconindex)) if(i not in dxindex)]
                        concindex=[concindex[i] for i in range(len(concindex)) if(i not in dxindex)]
                        accuracyindex=[accuracyindex[i] for i in range(len(accuracyindex)) if(i not in dxindex)]
                    ...
                    for index in range(len(sheetindex)):
                        ...
                        for i in range(len(file_data.row_values(8))):  
                            if file_data.row_values(8)[i]=="Raw File Name":
                                ...
                                conindex=i
                            elif file_data.row_values(8)[i]=="Theoretical Amount":
                                theoryconindex=i
                            elif file_data.row_values(8)[i]=="Amount Diff":
                                accuracyindex=i

                        # 未规范设置表格列名
                        if conindex == 0 or theoryconindex == 0 or accuracyindex == 0:
                        ...
                        for j in range(len(AMR_STD_sort)): 
                            ...
                            for i in range(nrows): # 循环原始数据中的每一行
                                if file_data.row_values(i)[nameindex][0:6] == AMR_STD_sort[j]: # 如果实验号命名方式匹配上，则在相应列表中添加相应数据              
                                    # print(file_data.row_values(i)[conindex]) 
                                    calconc.append(effectnum(file_data.row_values(i)[conindex],digits)) #添加实际浓度              
                                    theoryconc.append(effectnum(file_data.row_values(i)[theoryconindex],digits)) # 添加理论浓度
                    ...
                    norm=[] #化合物列表
                    norm_row=[] #化合物所在行
                    theoryconcindex=0  #理论浓度索引，岛津的数据格式决定每个化合物的浓度所在列一定是同一列，下同
                    concindex=0 #实际浓度索引
                    accuracyindex=0    #回收率索引
                    ...
                    for i in range(len(content[2])):  #第二行确定samplename和浓度所在列
                        if content[2][i]=="数据文件名":
                            ...
                            concindex=i 
                        elif content[2][i]=="标准浓度":
                            theoryconcindex=i 
                        elif content[2][i]=="精确度%":
                            accuracyindex=i 

                    # 未准确设置表头列名,直接返回并提示!
                    if theoryconcindex==0 or concindex==0 or accuracyindex==0:
                        error_message="未准确设置表头列名!"
                        return {"error_message":error_message}
                    ...
                    for k in range(len(norm)): # 循环化合物列表
                        ...
                        if k<len(norm)-1: #如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                            for i in range(norm_row[k],norm_row[k+1]): 
                                if "AMR-" in content[i][nameindex] and content[i][nameindex][0:6] not in AMR_STD:
                                    AMR_STD.append(content[i][nameindex][0:6])
                            ...
                            for j in range(len(AMR_STD_sort)): 
                                ...
                                for i in range(norm_row[k],norm_row[k+1]): 
                                    if content[i][nameindex][0:6] == AMR_STD_sort[j]: # 如果实验号命名方式匹配上，则在相应列表中添加相应数据  
                                        calconc.append(effectnum(content[i][concindex],digits)) #添加实际浓度              
                                        theoryconc.append(effectnum(content[i][theoryconcindex],digits)) # 添加理论浓度
                                        Accuracy.append(new_round(content[i][accuracyindex],2)) #添加回收率
    ...
    # 一 液质平台判断
    if platform=="液质":
        if len(AMR_dict[norm[0]][AMR_STD_sort[0]])!=13:
            error_message="LOQ验证曲线不为6条,请规范操作后再进行数据验证!"  
            return {"error_message":error_message}
    else:
        if len(AMR_dict[norm[0]][AMR_STD_sort[0]])!=6:
            error_message="LOQ验证曲线不为6条,请规范操作后再进行数据验证!"  
            return {"error_message":error_message}
...
# 2 上传多个数据文件
def LOQgeneral_multiplefileread(files,reportinfo,project,platform,Detectionplatform,manufacturers,Unit,digits,ZP_Method_precursor_ion,ZP_Method_product_ion,normAB,Number_of_compounds):
    ...
    if '.png' in files[0].name or ".jpg" in files[0].name or ".JPG" in files[0].name or ".PNG" in files[0].name or ".jpeg" in files[0].name:   
                ...
                if platform=="液质":
                    if manufacturers =="AB":
                        ...
                        if index==dataindex:
                            norm = []  # Thermo的原始数据格式为一个化合物一个sheet,获取每个sheet的名字,即为化合物名称
                            sheetindex = []  # 需要的化合物所在sheet索引列表
                            for index2 in range(len(data.sheet_names())):
                                norm.append(data.sheet_names()[index2].split("Sheet1")[1])
                                sheetindex.append(index2)
                        ...
                        # 未规范设置表格列名
                        if concindex == 0 or theoryconcindex == 0 or accuracyindex == 0:
                            error_message = "未规范设置文件列名，请检查并规范设置后重新提交数据!"
                            return {"error_message": error_message}
                        ...
                        for j in range(len(AMR_STD_sort)): 
                            ...
                            for i in range(nrows): # 循环原始数据中的每一行
                                if file_data.row_values(i)[nameindex][0:6] == AMR_STD_sort[j]: # 如果实验号命名方式匹配上，则在相应列表中添加相应数据              
                                    # 添加理论浓度(需避免重复添加)
                                    if effectnum(file_data.row_values(i)[theoryconcindex],digits) not in normdict[AMR_STD_sort[j]]:
                                        normdict[AMR_STD_sort[j]].append(effectnum(file_data.row_values(i)[theoryconcindex],digits))

                                    # 添加检测值
                                    normdict[AMR_STD_sort[j]].append(effectnum(file_data.row_values(i)[concindex],digits))

                                    # 添加回收率
                                    accuracyvalue = 100 - float(file_data.row_values(i)[accuracyindex])
                                    normdict[AMR_STD_sort[j]].append(new_round(accuracyvalue,2))
...
## 检出限数据上传读取
def LODfileread(files,reportinfo,project,platform,manufacturers,Unit,digits,ZP_Method_precursor_ion,ZP_Method_product_ion, normAB, Number_of_compounds):
    LOD_dict={}
...
def related_AMR(id,unit): 
    ...
    try:
        ...
        # 按AMR_dict中key的顺序输出queryset,ZMNs和NPP-C两个项目中化合物名称有重复的（MN和NMN，文拉法辛和去甲文拉法辛），model查询时需用name__startswith而不是name__contains
        if "ZMNs" in project:
            objs=[]
            for key in AMR_dict.keys():
                middle_objs = AMRpicture.objects.filter(reportinfo_id=id,name__startswith=key) #图片数据表     
                objs.extend(middle_objs)

        elif "NPP-C" in project:
            objs=[]
            for key in AMR_dict.keys():
                middle_objs = AMRpicture.objects.filter(reportinfo_id=id,name__startswith=key) #图片数据表     
                objs.extend(middle_objs)
        ...
        elif "NPP-E" in project:
            objs=[]
            for key in AMR_dict.keys():
                middle_objs = AMRpicture.objects.filter(reportinfo_id=id,name__startswith=key) #图片数据表     
                objs.extend(middle_objs)

        elif "NPP-H" in project:
            objs=[]
            for key in AMR_dict.keys():
                middle_objs = AMRpicture.objects.filter(reportinfo_id=id,name__startswith=key) #图片数据表     
                objs.extend(middle_objs)

        elif "血清中4种类固醇激素" in project:
            objs=[]
            for key in AMR_dict.keys():
                print(key)
                middle_objs = AMRpicture.objects.filter(reportinfo_id=id,name__startswith=key) #图片数据表     
            ...
            objs=[]
            for key in AMR_dict.keys():
                middle_objs = AMRpicture.objects.filter(reportinfo_id=id,name__contains=key) #图片数据表     
                objs.extend(middle_objs)
...
# try:
    ...
    print(AMRconclusion_dict)


    return {"AMRconclusion_dict":AMRconclusion_dict,"lodconclusion":lodconclusion}
