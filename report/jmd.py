...
def IntraP_fileread(files, reportinfo, namejmd, Detectionplatform, project, platform, manufacturers, Unit, digits, ZP_Method_precursor_ion, ZP_Method_product_ion, normAB, Number_of_compounds,Validation_Reason,Lid):
    ...
    for file in files:
        if platform == "液质":
            ...
            # 需安装python-docx包（安装docx会显示无法导入excepions包），pip install python-docx
            ...
            if manufacturers == "Agilent":
                ...
                for j in range(len(concindex)):
                    ...
                    for i in range(2, nrows):
                        if "PN-L" in file_data.row_values(i)[nameindex]:
                            if j < 1:
                                jmdnum += 1
                            low.append(effectnum(file_data.row_values(i)[concindex[j]], digits))
                        elif "PN-M" in file_data.row_values(i)[nameindex]:
                            median.append(effectnum(file_data.row_values(i)[concindex[j]], digits))
                        elif "PN-H" in file_data.row_values(i)[nameindex]:
    ...
    for k in range(norm_num):
        ...
        for index,file in enumerate(files):
            if platform == "液质":
                ...
                # 匹配原始数据中与精密度相关(实验号前含有"PJ-L"或"PJ-M"或"PJ-H")的行
                for i in range(2, nrows):
                    if "PJ-L" in file_data.row_values(i)[nameindex]:
                        if k < 1:
                            jmdnum += 1
                        low.append(file_data.row_values(i)[concindex[k]])
                    elif "PJ-M" in file_data.row_values(i)[nameindex]:
                        median.append(file_data.row_values(i)[concindex[k]])
                    elif "PJ-H" in file_data.row_values(i)[nameindex]:
