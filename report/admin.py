...
class RepeatprecisiongeneralAdmin(admin.ModelAdmin):
    inlines = [RepeatprecisiongeneralmethodInline,
               RepeatprecisiongeneraltextsInline]
...
class InterprecisiongeneralAdmin(admin.ModelAdmin):
    inlines = [InterprecisiongeneralmethodInline,
               InterprecisiongeneraltextsInline]
...
class SpecialAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """函数作用：使当前登录的用户只能看到自己负责的平台"""
        qs = super(SpecialAdmin, self).get_queryset(request)
        Usertable = User.objects.filter(first_name__endswith=request.user.first_name)
        currentid=""
        for i in Usertable:
            currentid = i.id
            print(currentid)

        Profiletable = Profile.objects.filter(user_id=currentid)
        currentDp=""
        for i in Profiletable:
            currentDp = i.Detectionplatform
            print(currentDp)

        if request.user.is_superuser:
            return qs
        else:
            if currentDp.endswith("临床服务二部"):
                return qs.filter(Q(Detectionplatform="内分泌检测平台")|Q(Detectionplatform="治疗药物检测平台"))
            elif currentDp.endswith("微量营养素检测平台"):
                return qs.filter(Detectionplatform="微量营养素检测平台")
            elif currentDp.endswith("遗传代谢病检测平台"):
                return qs.filter(Detectionplatform="遗传代谢病检测平台")
            elif currentDp.endswith("研发与创新平台"):
                return qs.filter(Detectionplatform="研发与创新平台")
...
class RepeatprecisionspecialAdmin(admin.ModelAdmin):
    inlines = [RepeatprecisionspecialmethodInline,
               RepeatprecisionspecialtextsInline]

    def has_module_permission(self, requset):
        return False

    def response_add(self, request, obj, post_url_continue=None):
        return redirect('/admin/report/special/%s' % obj.id)
...
class PTspecialAdmin(admin.ModelAdmin):
    inlines = [PTspecialmethodInline, PTspecialtextsInline,
               PTspecialacceptInline, PTspecialBooleanInline]
...
class Prepared_Sample_Stability_specialAdmin(admin.ModelAdmin):
    inlines = [Prepared_Sample_Stability_special_methodInline,
               Prepared_Sample_Stability_special_textsInline]
...
admin.site.register(Prepared_Sample_Stability_special,
                    Prepared_Sample_Stability_specialAdmin)
...
class YX_Method2Inline(admin.TabularInline):
    model = YX_Method2
    show_change_link = True
    can_delete = True
    extra = 0
...
class TestmethodAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """函数作用：使当前登录的用户只能看到自己负责的平台"""
        qs = super(TestmethodAdmin, self).get_queryset(request)
        Usertable = User.objects.filter(first_name__endswith=request.user.first_name)
        currentid=""
        for i in Usertable:
            currentid = i.id
    ...
    inlines = [ZP_MethodInline, ZP_MethodtextsInline,
               YX_MethodInline, YX_Method2Inline, YX_MethodtextsInline]
...
# 四 试剂耗材
# 1 Inline
class ReagentsTableInline(admin.TabularInline):
    model = ReagentsTable
    show_change_link = True
    can_delete = True
    extra = 0
...
class ReagentsInline(admin.TabularInline):
    model = Reagents
    show_change_link = True
    can_delete = True
    extra = 0
...
class ConsumablesInline(admin.TabularInline):
    model = Consumables
    show_change_link = True
    can_delete = True
    extra = 0
...
class Reagents_ConsumablesAdmin(admin.ModelAdmin):
    def get_queryset(self, request):
        """函数作用：使当前登录的用户只能看到自己负责的平台"""
        qs = super(Reagents_ConsumablesAdmin, self).get_queryset(request)
        Usertable = User.objects.filter(first_name__endswith=request.user.first_name)
        currentid=""
        for i in Usertable:
            currentid = i.id
    ...
    search_fields = ('Detectionplatform', 'name',)
    ordering = ('Detectionplatform',)
    inlines = [ReagentsTableInline, ReagentsInline, ConsumablesInline]
...
class Sample_PreparationAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        """函数作用：使当前登录的用户只能看到自己负责的平台"""
        qs = super(Sample_PreparationAdmin, self).get_queryset(request)
        Usertable = User.objects.filter(first_name__endswith=request.user.first_name)
        currentid=""
        for i in Usertable:
            currentid = i.id
