...
def Carryover_9sample_fileread(files, Detectionplatform, reportinfo, project, platform, manufacturers, Unit, digits, ZP_Method_precursor_ion, ZP_Method_product_ion, normAB, Number_of_compounds):
            ...
            if manufacturers == "Agilent":
                ...
                for j in range(len(norm)):
                    ...
                    for i in range(len(lines)):  # 循环原始数据中的每一行
                        ...
                        if "Carryover-C1" in lines[i][nameindex]:
                ...
                for j in range(len(norm)):
                    ...
                    if j < len(norm)-1:  # 如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                        for i in range(norm_row[j], norm_row[j+1]):
                            # 如果实验号命名方式匹配上，则在相应列表中添加相应数据
                            if "Carryover-C1" in file_data.row_values(i)[nameindex]:
                ...
                for i in range(len(norm)):
                    ...
                    for k in range(nrows):
                        # 如果实验号命名方式匹配上，则在相应列表中添加相应数据
                        if "Carryover-C1" in file_data.row_values(k)[nameindex]:
                            normlist.append(effectnum(file_data.row_values(k)[concindex], digits))
                        elif "Carryover-C2" in file_data.row_values(k)[nameindex]:
                            normlist.append(effectnum(file_data.row_values(k)[concindex], digits))
                        elif "Carryover-C3" in file_data.row_values(k)[nameindex]:
                            normlist.append(effectnum(file_data.row_values(k)[concindex], digits))

                    Carryover_dict[Systermlist[index]][norm[i]] = normlist
                ...
                for j in range(len(norm)):
                    ...
                    if j < len(norm)-1:  # 如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                        for i in range(norm_row[j], norm_row[j+1]):
                            # 如果实验号命名方式匹配上，则在相应列表中添加相应数据
                            if "Carryover-C1" in content[i][nameindex]:
    ...
    for key, value in Carryover_enddict.items():
        for r, c in value.items():
            ...
            # bias = new_round(abs(float(C3mean)-float(C1mean))/float(C1mean)*100, 2) # 取绝对值
            bias = new_round((float(C3mean)-float(C1mean))/float(C1mean)*100, 2) # 不取绝对值
...
def Carryover_21sample_fileread(files, Detectionplatform, reportinfo, project, platform, manufacturers, Unit, digits, ZP_Method_precursor_ion, ZP_Method_product_ion, normAB, Number_of_compounds):
    '''
    注释1:csv,txt,xlsx,docx 4种格式数据读取完毕后,需要生成一个字典Carryover_dict,数据格式如下：
    print(Carryover_dict):
    ...
    for file in files:
        if platform == "液质":
            ...
            for key, value in Carryover_dict.items():
                ...
                # 添加X1(2、3、10、11、12样品的均值)
                Carryover_dict[key].append(new_round(np.mean(X1), 2))
                # 添加X2(6、9、15、18、21样品的均值)
                Carryover_dict[key].append(new_round(np.mean(X2), 2))
                
                X1mean = new_round(np.mean(X1), 2)
                X2mean = new_round(np.mean(X2), 2)
                Carryover_dict[key].append(new_round(float(X2mean)-float(X1mean), 2))  # 添加X2-X1

                X1sd = new_round(sd(X1), 2)
                # 添加SD1(2、3、10、11、12样品的标准差)
                Carryover_dict[key].append(X1sd)
                # 添加3SD1(2、3、10、11、12样品的标准差)
                Carryover_dict[key].append(new_round(3*float(X1sd), 2))

                # 判断是否通过(X2-X1<3SD1)
                if float(X2mean)-float(X1mean) < 3*float(X1sd):
                    pass
                else:
                    Carryover_judgenum += 1
    ...
    if Carryover_judgenum == 0:
        # insert_list = []
        # for key, value in Carryover_dict.items():
        #     insert_list.append(Carryover2(reportinfo=reportinfo, norm=key, L01=value[0], L02=value[1], L03=value[2], H04=value[3], H05=value[4],L06=value[5], H07=value[6], H08=value[7], L09=value[8], 
        #     L10=value[9], L11=value[10], L12=value[11], H13=value[12], H14=value[13], L15=value[14],H16=value[15], H17=value[16], L18=value[17], H19=value[18], H20=value[19], L21=value[20], X1=value[21], X2=value[22], SD1=value[24]))
...
def related_Carryover(id):
    ...
    text_general = Carryovergeneraltexts.objects.filter(
        carryovergeneral=general_2)
    ...
    try:
        ...
        for i in Carryover_norm:
            ...
            for j in Carryover_systermnum:
                ...
                for item in middle_table:
                    ...
                    elif list(Digitsdict.values())[0] == None:
                        middle_list.append(item.C1_1)
                        middle_list.append(item.C2_1)
                        middle_list.append(item.C3_1)
                        middle_list.append(item.C1_2)
        ...
        if len(Carryover_systermnum) == 1:
            ...
            elif len(d) == 48:  # 48，拆分为七个table
                d_reshape = {}
                d_reshape["table1"] = dict(list(d.items())[:len(d)//7+1])
                d_reshape["table2"] = dict(list(d.items())[len(d)//7+1:len(d)//7*2+2])
                d_reshape["table3"] = dict(list(d.items())[len(d)//7*2+2:len(d)//7*3+3])
                d_reshape["table4"] = dict(list(d.items())[len(d)//7*3+3:len(d)//7*4+4])
                d_reshape["table5"] = dict(list(d.items())[len(d)//7*4+4:len(d)//7*5+5])
                d_reshape["table6"] = dict(list(d.items())[len(d)//7*5+5:len(d)//7*6+6])
                d_reshape["table7"] = dict(list(d.items())[len(d)//7*6+6:])
