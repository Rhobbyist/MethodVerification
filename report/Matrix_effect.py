...
def Matrix_effect_fileread(files,Detectionplatform, reportinfo,project,platform,manufacturers,digits,ZP_Method_precursor_ion,ZP_Method_product_ion,normAB, Number_of_compounds):
        ...
        if platform=="液质":
            if manufacturers =="Agilent":
                ...
                if Detectionplatform=="治疗药物检测平台":
                    ...
                    Areaindex=[Areaindex[i] for i in range(len(Areaindex)) if(i not in dxindex)]
                    ISAreaindex=[ISAreaindex[i] for i in range(len(ISAreaindex)) if(i not in dxindex)]
                    AreaRatioindex=[AreaRatioindex[i] for i in range(len(AreaRatioindex)) if(i not in dxindex)]
                ...
                for k in range(len(norm)):  
                    ...
                    STDlist=[]
                    for i in range(len(lines)): 
                        if "STD" in lines[i][nameindex] and "S1" not in lines[i][nameindex] and lines[i][nameindex][0:5] not in STDlist:
                ...
                # 未规范设置表格列名
                if nameindex == 0 or Areaindex == 0 or ISAreaindex == 0:
                ...
                for k in range(len(norm)):  
                    ...
                    if k < len(norm)-1:  # 如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                        for i in range(norm_row[k], norm_row[k+1]):
                            print(file_data.row_values(i)[nameindex])
                            if "PS" in file_data.row_values(i)[nameindex] and file_data.row_values(i)[nameindex][0:4] not in PSlist:
                                PSlist.append(file_data.row_values(i)[nameindex][0:4])

                            elif ("STD-L" in file_data.row_values(i)[nameindex] or "STD-M" in file_data.row_values(i)[nameindex] or "STD-H" in file_data.row_values(i)[nameindex]) and file_data.row_values(i)[nameindex][0:5] not in STDlist:
                                STDlist.append(file_data.row_values(i)[nameindex][0:5])

                            elif "MIX" in file_data.row_values(i)[nameindex] and file_data.row_values(i)[nameindex][0:6] not in MIXlist:
                                MIXlist.append(file_data.row_values(i)[nameindex][0:6])

                        # 循环病人基质样本列表
                        for j in PSlist:
                            middlelist=[] 
                            for i in range(norm_row[k], norm_row[k+1]):
                                if j in file_data.row_values(i)[nameindex]:
                                    middlelist.append(effectnum(file_data.row_values(i)[Areaindex],digits))
                                    middlelist.append(effectnum(file_data.row_values(i)[ISAreaindex],digits))
                ...
                for index in range(len(sheetindex)):
                    ...
                    for i in range(len(file_data.row_values(8))):  
                        if file_data.row_values(8)[i]=="Raw File Name":
                            nameindex=i
                        elif file_data.row_values(8)[i]=="Peak Area":
                            Areaindex=i
                        elif file_data.row_values(8)[i]=="ISTD Area":
                            ISAreaindex=i
                        elif file_data.row_values(8)[i]=="ION Ratio":
                            AreaRatioindex=i
                    ...
                    Matrix_effect_dict[norm[index]]=normdict
                ...
                for i in range(len(content[2])):  #第三行确定samplename和浓度所在列
                    if content[2][i]=="数据文件名":
                        ...
                        Areaindex=i 
                    elif content[2][i]=="内部面积":
                        ISAreaindex=i 
                    elif content[2][i]=="面积比":
                        AreaRatioindex=i 
                ...
                for k in range(len(norm)):  
                    ...
                    if k < len(norm)-1:  # 如果不是最后一个化合物，索引为该化合物所在行到后一个化合物所在行
                        for i in range(norm_row[k], norm_row[k+1]):
                            if "PS" in content[i][nameindex] and content[i][nameindex][0:4] not in PSlist:
                                PSlist.append(content[i][nameindex][0:4])

                            elif "STD" in content[i][nameindex] and "S1" not in content[i][nameindex] and content[i][nameindex][0:5] not in STDlist:
                                STDlist.append(content[i][nameindex][0:5])

                            elif "MIX" in content[i][nameindex] and content[i][nameindex][0:6] not in MIXlist:
                                MIXlist.append(content[i][nameindex][0:6])

                        # 循环病人基质样本列表
                        for j in PSlist:
                            middlelist=[] 
                            for i in range(norm_row[k], norm_row[k+1]):
                                if j in content[i][nameindex]:
                                    middlelist.append(effectnum(content[i][Areaindex],digits))
                                    middlelist.append(effectnum(content[i][ISAreaindex],digits))
                                    middlelist.append(new_round(float(content[i][AreaRatioindex]),3))
                            normdict[j]=middlelist
                        ...
                        for i in range(norm_row[k], len(content)-1):
                            ...
                            elif "STD" in content[i][nameindex] and content[i][nameindex][0:5] not in STDlist and "S1" not in content[i][nameindex] and "lcd" not in content[i][nameindex] :
                                STDlist.append(content[i][nameindex][0:5])
                ...
                for k in range(len(tableindex)):
                    ...
                    STDlist=[]
                    for i in range(len(rowdatagatherlist)): 
                        if "STD" in rowdatagatherlist[i][nameindex] and "S1" not in rowdatagatherlist[i][nameindex] and rowdatagatherlist[i][nameindex][0:5] not in STDlist:
        ...
        for r,c in value.items():
            if "PS-" in r or "STD-" in r:  # 判断是否是混合样本
                ...
                for i in [2,5,8]: # 2,5,8为三个Area/IS Area值在字典中的索引
                    ...
                    # bias=new_round(abs(num1-num2)/num2*100,2)  取绝对值
                    bias=new_round((num1-num2)/num2*100,2)  # 不取绝对值
...
def related_Matrix_effect(id):
    ...
    try:
        ...
        Matrixeffect_endreport_conclusion="混合样本中" + "、" .join(list(Matrixeffect_dict.keys()))+"的相应值(a)与高低浓度样本和病人样本中"+"、" .join(list(Matrixeffect_dict.keys()))+"响应值的均值(b)差异均在±20%范围内，说明无相对基质效应。"
